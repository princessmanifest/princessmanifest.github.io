...
# Step 2: Filter for Midwest only
midwest_df = df[df['region'] == 'Midwest'].copy()
...
midwest_df['object_words'] = midwest_df['detected_objects'].apply(extract_all_words)
midwest_df['object_words'] = midwest_df['object_words'].apply(lambda words: [w for w in words if len(w) > 1])
...
for words, score in zip(midwest_df['object_words'], midwest_df['Haunted Score']):
    for word in words:
        word_freq[word] += 1
        if pd.notna(score):
...
midwest_wordcloud_cleaned_df = pd.DataFrame([{
    "word": word,
...
midwest_wordcloud_cleaned_df = midwest_wordcloud_cleaned_df.sort_values(by='frequency', ascending=False)

midwest_wordcloud_cleaned_df = midwest_wordcloud_cleaned_df.rename(columns={
    'word': 'text',
    'frequency': 'frequency',
...
# Select and export
midwest_wordcloud_cleaned_df[['text', 'frequency', 'score']].to_json(
    "midwest_wordcloud_d3.json", 
    orient='records', 
    indent=2
)
