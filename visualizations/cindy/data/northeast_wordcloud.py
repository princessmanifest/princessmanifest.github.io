...
# Step 2: Filter for Northeast only
northeast_df = df[df['region'] == 'Northeast'].copy()
...
northeast_df['object_words'] = northeast_df['detected_objects'].apply(extract_all_words)
northeast_df['object_words'] = northeast_df['object_words'].apply(lambda words: [w for w in words if len(w) > 1])
...
for words, score in zip(northeast_df['object_words'], northeast_df['Haunted Score']):
    for word in words:
        word_freq[word] += 1
        if pd.notna(score):
...
northeast_wordcloud_cleaned_df = pd.DataFrame([{
    "word": word,
...
northeast_wordcloud_cleaned_df = northeast_wordcloud_cleaned_df.sort_values(by='frequency', ascending=False)

northeast_wordcloud_cleaned_df = northeast_wordcloud_cleaned_df.rename(columns={
    'word': 'text',
    'frequency': 'frequency',
...
# Select and export
northeast_wordcloud_cleaned_df[['text', 'frequency', 'score']].to_json(
    "northeast_wordcloud_d3.json", 
    orient='records', 
    indent=2
)
