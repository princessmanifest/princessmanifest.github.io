...
south_df['object_words'] = south_df['detected_objects'].apply(extract_all_words)
south_df['object_words'] = south_df['object_words'].apply(lambda words: [w for w in words if len(w) > 1])
...
for words, score in zip(south_df['object_words'], south_df['Haunted Score']):
    for word in words:
        word_freq[word] += 1
        if pd.notna(score):
...
south_wordcloud_cleaned_df = pd.DataFrame([{
    "word": word,
...
south_wordcloud_cleaned_df = south_wordcloud_cleaned_df.sort_values(by='frequency', ascending=False)

south_wordcloud_cleaned_df = south_wordcloud_cleaned_df.rename(columns={
    'word': 'text',
    'frequency': 'frequency',
...
# Select and export
south_wordcloud_cleaned_df[['text', 'frequency', 'score']].to_json(
    "south_wordcloud_d3.json", 
    orient='records', 
    indent=2
)
